...
def test_csv_processor_creates_four_files(tmp_path):
    ...
    data = pd.DataFrame({
        "system_id": [1, 2],
        "test_id": [None, None],  # New records
        "name": ["Test A", "Test B"]
    })
    
    submission = Submission(
        data_tables={"tbl_test": data},
        schema=schema,
...
def test_csv_processor_handles_foreign_keys(tmp_path):
        ...
        build_table(
            ...
            columns={
                ...
                "lookup_id": build_column(
                    "tbl_main", "lookup_id", is_fk=True, 
                    fk_table_name="tbl_lookup", class_name="TblLookup",
                    data_type="integer"
                ),
    ...
    main_data = pd.DataFrame({
        "system_id": [1],
        "main_id": [None],
        "lookup_id": [100]  # References system_id in lookup table
    })
...
def test_csv_processor_format_compatibility():
    ...
    expected_columns = {
        ...
        "recordvalues": ["class_name", "system_id", "public_id", "column_name", 
                        "column_type", "fk_system_id", "fk_public_id", "column_value"]
    }
